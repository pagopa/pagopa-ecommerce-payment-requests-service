...
class NodPerPspClientTests {
    ...
    fun `should return verify payment response given valid payment notice`() = runTest {
        ...
        given(responseSpec.bodyToMono(NodoVerificaRPTRisposta::class.java)).willReturn(Mono.just(verificaRPTRisposta))
        ...
        assertThat(testResponse?.nodoVerificaRPTRisposta?.datiPagamentoPA?.importoSingoloVersamento).isEqualTo(
            importoSingoloVersamento
        )
        assertThat(testResponse?.nodoVerificaRPTRisposta?.datiPagamentoPA?.causaleVersamento).isEqualTo(
            causaleVersamento
        )
    }
