...
class IdempotencyKeyTests {
  ...
  fun `should throw invalid fiscal code`() {
    val exception =
      assertThrows<IllegalArgumentException> { IdempotencyKey(INVALID_FISCAL_CODE, VALID_KEY_ID) }
  ...
  fun `should throw invalid key id`() {
    val exception =
      assertThrows<IllegalArgumentException> { IdempotencyKey(VALID_FISCAL_CODE, INVALID_KEY_ID) }
